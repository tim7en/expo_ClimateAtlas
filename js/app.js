...
const elements = {
  ...
  libraryRotateLeft: document.querySelector("#library-rotate-left"),
  libraryRotateRight: document.querySelector("#library-rotate-right"),
...
function renderClimateMapCard(sourceDocument, visibleIndex) {
  ...
  const noteTag = hasLibraryNote(sourceDocument.id)
    ...
    : "";
  const viewerTag = `
    <div class="card-draft-tag climate-map-viewer-chip">
...
function formatLibraryAiModelRun(model, durationMs) {
  const details = [];
  ...
  if (duration > 0) {
    details.push(`${formatLibraryAiDuration(duration)}`);
  }
...
async function explainSelectedLibraryPage() {
  ...
  try {
    ...
    finalStatusMessage = `English explanation generated for page ${pageNumber}${formatLibraryAiModelRun(payload?.model, payload?.durationMs)}.`;
  } catch (error) {
    if (error?.name === "AbortError") {
      return;
    }
...
function normalizeLibraryRotation(rotation) {
  const normalized = Number(rotation) || 0;
  return ((Math.round(normalized / 90) * 90) % 360 + 360) % 360;
}

function getLibraryPageRotation(pdfPage) {
  const pageRotation = normalizeLibraryRotation(pdfPage?.rotate || 0);
  return normalizeLibraryRotation(pageRotation + state.libraryRotation);
}
...
function updateLibraryPdfControls() {
  ...
  const rotationLabel = state.libraryRotation
    ? ` · rotated ${state.libraryRotation}deg`
    : "";
  elements.libraryPage.textContent = hasDocument
    ? `Page ${state.libraryPage} of ${state.libraryPageCount} · ${Math.round(state.libraryZoom * 100)}%${rotationLabel}`
    : "Page -- of --";
  ...
  if (elements.libraryRotateLeft) {
    elements.libraryRotateLeft.disabled = !hasDocument;
  }
  if (elements.libraryRotateRight) {
    elements.libraryRotateRight.disabled = !hasDocument;
  }
  syncLibraryAiPanel();
}

function rotateLibraryPdf(deltaDegrees) {
  if (!state.libraryPdfDocument) {
    return;
  }

  state.libraryRotation = normalizeLibraryRotation(state.libraryRotation + deltaDegrees);
...
async function renderLibraryPdfPage(requestedPage = state.libraryPage, retryCount = 0) {
  ...
  try {
    ...
    const pageRotation = getLibraryPageRotation(page);
...
function wireEvents() {
  ...
  elements.libraryRotateLeft?.addEventListener("click", () => {
    rotateLibraryPdf(-90);
  });

  elements.libraryRotateRight?.addEventListener("click", () => {
    rotateLibraryPdf(90);
  });
